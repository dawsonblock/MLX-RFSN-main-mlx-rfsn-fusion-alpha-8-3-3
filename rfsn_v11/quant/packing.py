"""
Bit-packing utilities for RFSN v11 compressed storage.

Ported from mlx-turboquant-main/mlx_turboquant/packing.py.

Fix applied: pack_indices Python loop replaced with mx.sum vectorized reduction.
Original code had:
    packed = shifted[..., 0]
    for i in range(1, vals_per_int):
        packed = packed | shifted[..., i]
which builds up to 16 sequential Python-level graph nodes at 2-bit.
Replacement:
...
def pack_indices(indices: mx.array, bits: int) -> mx.array:
    """Pack low-bit indices into uint32 arrays (vectorized, no Python loop).
    ...
    # This replaces the Python loop: for i in range(1, vals_per_int): packed |= shifted[..., i]
    packed = mx.sum(shifted, axis=-1).astype(mx.uint32)
