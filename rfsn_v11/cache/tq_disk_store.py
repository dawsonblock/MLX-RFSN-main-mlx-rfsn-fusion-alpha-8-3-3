"""TQ-native serialization for rfsn_v11 disk cache.

Ported from vmlx-main/vmlx_engine/tq_disk_store.py.

Stores TurboQuantKVCache compressed data (packed indices, norms, metadata)
directly to safetensors without decompressing to float16 first.  Non-TQ
layers (KVCache, MambaCache, QuantizedKVCache) are serialised via their
``.state`` property.

Compression ratio: ~26x vs float16 (40 KB vs 1 MB per 100 tokens x 8 heads x
128 dim).

Serialised metadata includes ``__cache_version__`` from ``.version`` so that a
record written with a different CACHE_VERSION is hard-rejected on load.

Format — safetensors tensors:
...
Format — safetensors metadata (string key-value):
  __tq_native__ = "true"
  __num_layers__ — total layer count
  __cache_version__ — CACHE_VERSION from .version
...
def is_tq_compressed_cache(cache: List[Any]) -> bool:
    """Return True if any layer is a TurboQuantKVCache with compressed data.

    Checks that compress() has been called (``_compressed_keys`` is set),
    meaning native TQ serialisation is possible.
    """
    for c in cache:
        if (
            type(c).__name__ == _TQ_CLASS_NAME
...
def serialize_tq_cache(
    ...
    For TurboQuantKVCache layers: extracts ``_compressed_keys`` /
    ``_compressed_values`` directly (~26x smaller than ``.state``).
    For other layers (KVCache, MambaCache, etc.): uses standard state
    extraction.

    The serialised metadata includes ``__cache_version__`` = CACHE_VERSION
    so a deserialiser can hard-fail on version mismatch.
    ...
    meta: Dict[str, str] = {
        ...
        "__cache_version__": str(CACHE_VERSION),
    }
    tq_count = 0
    non_tq_count = 0
    ...
    logger.info(
        "TQ-native serialize: %d TQ layers (compressed), %d standard layers",
        tq_count,
        non_tq_count,
    )
    return tensors, meta
...
def deserialize_tq_cache(
    ...
    Hard-fails (raises ValueError) if ``__cache_version__`` in metadata does
    not match CACHE_VERSION — loading a mismatched cache must never silently
    reconstruct incorrect tensors.
    ...
    convert back to TurboQuantKVCache using the model's ``make_cache()``
    template.
    ...
    Raises:
        ValueError: If CACHE_VERSION mismatches or metadata validation fails.
    """
    if not HAS_MLX:
        raise RuntimeError("MLX required for TQ deserialization")

    # Hard version check — must match before any tensor allocation.
    stored_version = metadata.get("__cache_version__", None)
    if stored_version is not None:
        try:
            stored_int = int(stored_version)
        except (TypeError, ValueError):
            raise ValueError(
                f"__cache_version__ {stored_version!r} is not a valid integer"
            )
        if stored_int != CACHE_VERSION:
            raise ValueError(
                f"Cache version mismatch: on-disk={stored_int}, "
                f"current={CACHE_VERSION}. Hard-failing to prevent "
                "incorrect tensor reconstruction."
            )

    # Validate TQ metadata before any decode allocation.
    try:
        from .cache_record_validator import validate_tq_native_metadata
    except Exception:
        validate_tq_native_metadata = None  # type: ignore
    ...
    try:
        ...
        raise RuntimeError("mlx_lm required for TQ deserialization") from e
    ...
    logger.info(
        "TQ-native deserialize: %d TQ decoded, %d standard, %d total layers",
        tq_decoded,
        standard_loaded,
        num_layers,
    )
...
def _serialize_tq_layer(
    ...
    # EncodedKeys tensors (4 mx.array fields)
...
def _serialize_cache_list_layer(
    ...
    CacheList wraps a list of sub-caches (``.caches`` attribute). Each
    sub-cache can be TQ, KVCache, or cumulative (MambaCache). We serialize
    each sub-cache independently using the appropriate path.
    """
    ...
    for j, sub in enumerate(sub_caches):
        ...
        if (
            sub_cls == _TQ_CLASS_NAME
            and getattr(sub, "_compressed_keys", None) is not None
        ):
            # TQ sub-cache: serialize compressed data with cl_ prefix
            ...
            if sub_meta:
                meta[f"__cl_{i}_{j}_meta__"] = json.dumps(
                    [str(x) for x in sub_meta]
                    if isinstance(sub_meta, tuple)
                    else str(sub_meta)
                )


# =============================================================================
# Internal: CacheList deserialization
# =============================================================================
...
def _deserialize_cache_list_layer(
    ...
    Returns a CacheList wrapping KVCache objects, or a single KVCache as
    fallback.
    """
    from mlx_lm.models.cache import KVCache
    ...
    for j in range(sub_count):
        ...
        if sub_cls == _TQ_CLASS_NAME and f"{prefix}_ck_indices_packed" in tensors:
            ...
            kv = KVCache()
            try:
                from jang_tools.turboquant.cache import EncodedKeys, EncodedValues  # type: ignore
                from jang_tools.turboquant.pipeline import decode_keys, decode_values  # type: ignore
                ...
                from jang_tools.turboquant.cache import TurboQuantKVCache as _TQ_CL  # type: ignore
                ...
                _tq_cl = _TQ_CL(
                    key_dim=_key_dim, value_dim=_val_dim,
                ...
                kv.offset = _parse_bounded_int(
                    metadata, f"__{prefix}_offset__", default=0, lo=0, hi=2_000_000
                )
            except Exception as e:
                logger.warning("CacheList TQ sub %d decode failed: %s", j, e)
            sub_caches.append(kv)
        elif f"cl_{i}_{j}_keys" in tensors:
            kv = KVCache()
            kv.keys = tensors.get(f"cl_{i}_{j}_keys")
            kv.values = tensors.get(f"cl_{i}_{j}_values")
            sub_meta_str = metadata.get(f"__cl_{i}_{j}_meta__", "")
            if sub_meta_str:
                try:
                    kv.offset = _parse_meta_offset_str(sub_meta_str, f"cl_{i}_{j}_meta")
                except (json.JSONDecodeError, ValueError, IndexError):
                    kv.offset = (
                        kv.keys.shape[2]
                        if kv.keys is not None and kv.keys.ndim >= 3
                        else 0
                    )
            else:
                kv.offset = (
...
def _deserialize_tq_layer(
    ...
    use ``_recompress_to_tq()`` with the model's ``make_cache()`` template to
    convert back to TurboQuantKVCache.
    """
    try:
        from jang_tools.turboquant.cache import EncodedKeys, EncodedValues  # type: ignore
        from jang_tools.turboquant.pipeline import decode_keys, decode_values  # type: ignore
    ...
    # Decode compressed data to float16 using a TurboQuantKVCache encoder.
    offset = _parse_bounded_int(
        metadata, f"__{prefix}_offset__", default=0, lo=0, hi=2_000_000
    )
    key_dim = _parse_bounded_int(
        metadata, f"__{prefix}_key_dim__", default=128, lo=1, hi=262144
    )
    value_dim = _parse_bounded_int(
        metadata, f"__{prefix}_value_dim__", default=128, lo=1, hi=262144
    )
    key_bits = _parse_bounded_int(
        metadata, f"__{prefix}_key_bits__", default=3, lo=1, hi=8
    )
    value_bits = _parse_bounded_int(
        metadata, f"__{prefix}_value_bits__", default=3, lo=1, hi=8
    )
    sink_tokens = _parse_bounded_int(
        metadata, f"__{prefix}_sink_tokens__", default=0, lo=0, hi=2_000_000
    )

    try:
        from jang_tools.turboquant.cache import TurboQuantKVCache as _TQ  # type: ignore
        tq = _TQ(
            key_dim=key_dim,
            value_dim=value_dim,
            key_bits=key_bits,
...
def _deserialize_quantized_kv(
    ...
    Attempts QuantizedKVCache.from_state(); falls back to dequantizing into a
    plain KVCache if unavailable.
    """
    from mlx_lm.models.cache import KVCache
    ...
    try:
        ...
        logger.warning("Quantized layer %d reconstruct failed: %s", i, e)
        return KVCache()
...
def _parse_offset(metadata: Dict[str, str], i: int) -> Optional[int]:
    """Parse offset from meta_state metadata string."""
    meta_str = metadata.get(f"__layer_{i}_meta__", "")
    if not meta_str:
        return None
    try:
...
def _parse_meta_offset_str(meta_str: str, label: str) -> int:
    """Parse the first element of a JSON list as an integer offset."""
...
def _parse_bounded_int(
    ...
    *,
    ...
    """Parse an integer from metadata with bounds checking.

    Args:
        metadata: String metadata dict.
        key: Metadata key to read.
        default: Value to use if key is absent.
    ...
    Raises:
        ValueError: If value is non-integer or out of bounds.
    """
