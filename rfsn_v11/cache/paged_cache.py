"""Paged KV Cache Manager for rfsn_v11.

Ported from vmlx-main/vmlx_engine/paged_cache.py.
vmlx-specific imports (jang_tools, vmlx_engine models) are removed or stubbed.
MemoryGuard eviction callback is wired into PagedCacheManager.

Key components:
- CacheBlock / KVCacheBlock: Metadata for each cache block with doubly linked
  list pointers for O(1) LRU operations.
...
- PagedCacheManager: Main manager with block allocation, prefix caching, COW,
  and optional MemoryGuard eviction.
...
def compute_block_hash(
    ...
    Args:
        ...
        extra_keys: Additional keys (e.g. LoRA IDs, vision embeddings).
            MLX array hashing requires mlx to be installed; plain Python
            objects are hashed via their string representation.
    ...
    if extra_keys is not None:
        def _hash_extra(obj: Any) -> None:
            try:
                import mlx.core as mx
                if isinstance(obj, mx.array):
                    import numpy as np
                    hasher.update(bytes(str(obj.shape), "utf-8"))
...
class CacheBlock:
    ...
    Each block represents a fixed number of tokens (block_size) worth of KV
    cache data.  Blocks can be shared across requests via reference counting
    for prefix caching.
    ...
    def is_full(self, block_size: int) -> bool:
        """Return True if the block is at capacity."""
        return self.token_count >= block_size

    def is_shared(self) -> bool:
        """Return True if the block is shared (ref_count > 1)."""
...
class FreeKVCacheBlockQueue:
    ...
    def __init__(self, blocks: List[CacheBlock]) -> None:
        """Initialise queue with all blocks as free.
    ...
    def remove(self, block: CacheBlock) -> None:
        ...
        Raises:
            RuntimeError: If block is not in the queue.
        """
    ...
    def get_all_free_blocks(self) -> List[CacheBlock]:
        """Return all free blocks (for testing / diagnostics)."""
        result = []
        curr = self.fake_head.next_free_block
...
class BlockHashToBlockMap:
    ...
    Follows vLLM's design where the same hash can map to multiple blocks
    (for different KV cache groups in hybrid models).
    """

    def __init__(self) -> None:
        """Initialise an empty hash-to-block map."""
        self._cache: Dict[BlockHash, Any] = {}

    def get_block(self, block_hash: BlockHash) -> Optional[CacheBlock]:
        """Return any block with the given hash, or None."""
    ...
    def insert(self, block_hash: BlockHash, block: CacheBlock) -> None:
        """Insert a block into the map, supporting multiple blocks per hash."""
    ...
    def pop(self, block_hash: BlockHash, block_id: int) -> Optional[CacheBlock]:
        """Remove and return a specific block from the map."""
...
class BlockTable:
    ...
    def copy(self, new_request_id: str) -> BlockTable:
        """Create a copy with a new request ID."""
        return BlockTable(
...
class PagedCacheManager:
    ...
    - Optional MemoryGuard integration: when ``eviction_callback`` is supplied,
      the manager calls it whenever the estimated in-memory block footprint
      exceeds ``max_memory_gb``.

    Args:
        block_size: Number of tokens per block (default 64).
        max_blocks: Maximum number of blocks (default 1000).
        enable_caching: Whether to enable prefix caching (default True).
        disk_store: Optional L2 disk store for block persistence.
        eviction_callback: Callable invoked when block memory exceeds
            ``max_memory_gb``.  Receives the target bytes to free and should
            return the bytes actually freed.
        max_memory_gb: Soft memory ceiling (GB) for eviction trigger.
            Ignored when ``eviction_callback`` is None.
        block_bytes: Estimated bytes per block used for memory accounting.
            Defaults to 0 (eviction based purely on block count vs. limit).
    """
    ...
    def __init__(
        ...
        if max_blocks < 2:
            raise ValueError(
                f"max_blocks must be >= 2 (1 reserved for null block), got {max_blocks}"
            )
        ...
        logger.info(
            "PagedCacheManager initialized: block_size=%d, max_blocks=%d, "
            "max_tokens=%d",
            block_size,
            max_blocks,
    ...
    def _check_memory_guard(self) -> None:
        """Trigger MemoryGuard eviction if block footprint exceeds the limit."""
        if self._memory_guard is None:
            return
        estimated = len(self.allocated_blocks) * self._block_bytes
        self._memory_guard.enforce_safety(estimated_cache_bytes=estimated)
    ...
    def _maybe_evict_cached_block(self, block: CacheBlock) -> bool:
        ...
        If a disk store is configured, persists the block before freeing RAM.
    ...
    def free_block(self, block_id: int) -> bool:
        ...
        with self._lock:
            if block_id not in self.allocated_blocks:
                logger.warning("Attempted to free unknown block: %d", block_id)
                return False
    ...
    def cache_full_blocks(
        ...
        Args:
            ...
            extra_keys: Additional keys for hashing.
        """
        if not self.enable_caching:
            return
    ...
    def get_computed_blocks(
        ...
        Args:
            ...
            extra_keys: Additional keys for chain hash computation.
    ...
    def _promote_from_disk(
        ...
        and registers it in the hash cache.
    ...
    def compute_block_hash(self, tokens: List[int]) -> str:
        """Compute a legacy string hash for a block (SHA-256 hex, no chain)."""
        return hashlib.sha256(
            bytes(str(tuple(tokens)), "utf-8")
        ).hexdigest()

    def find_cached_block(self, tokens: List[int]) -> Optional[CacheBlock]:
        """Find a cached block by legacy string hash.

        Args:
            tokens: Token IDs to look up.

        Returns:
            Cached CacheBlock if found, None otherwise.
        """
        with self._lock:
    ...
    def register_block_hash(self, block: CacheBlock, tokens: List[int]) -> None:
        """Register a block's legacy hash for deduplication."""
        with self._lock:
            hash_value = self.compute_block_hash(tokens)
    ...
    def release_request_refs(self, block_table: Optional[BlockTable]) -> int:
        ...
        Implements "cached but free" semantics: decrement one ref per
        block-table entry; when a block reaches ref_count==0, keep hash
        entries intact but move to the free LRU queue.
        ...
        with self._lock:
            ...
            for block_id in block_table.block_ids:
                ...
                if block.ref_count <= 0:
                    logger.warning(
                        "release_request_refs: block %d already has ref_count=%d; skipping",
                        block_id,
                        block.ref_count,
                    )
                    continue
    ...
    def fork_block_table(
        ...
        with self._lock:
            ...
            logger.debug(
                "Forked block table: %s -> %s, blocks=%d",
    ...
    def _cow_copy_block(self, source_block: CacheBlock) -> Optional[CacheBlock]:
        ...
        logger.debug(
            "COW copy: block %d -> %d", source_block.block_id, new_block.block_id
        )
    ...
    def evict_lru_blocks(self, num_blocks: int) -> int:
        ...
        with self._lock:
            ...
            if evicted > 0:
                logger.info("Evicted %d LRU blocks from cache", evicted)
    ...
    def get_memory_usage(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "cache_hit_rate": (
                    stats.cache_hits / total_hits if total_hits > 0 else 0
                ),
